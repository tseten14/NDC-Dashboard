...
After generation, a deterministic resolver maps cited identifiers to URLs. If the model omits identifiers, the resolver tries numeric matching (absolute tolerance $0.06$ or relative $2.5\%$) and, failing that, conservative keyword inference. Any paragraph that still contains a number not in the ledger is marked as having unverified numbers and the answer's confidence is treated as low (Table~\ref{fig:ai-pipeline}). In a diplomatic setting, an ungrounded but fluent answer is worse than no answer: a non-expert user cannot reliably distinguish a well-supported claim from a plausible fabrication without an explicit source trail. The ledger makes that trail mechanical.
...
\begin{table}[t]
  ...
  \begin{tabular}{ccccc}
    ...
    Dashboard state & $\rightarrow$ & Typed fact ledger & $\rightarrow$ & LLM (quote-only) \\
     & & & $\rightarrow$ & Citation resolver \\
     & & & $\rightarrow$ & Cited answer + confidence \\
    \bottomrule
  \end{tabular}
\end{table}
