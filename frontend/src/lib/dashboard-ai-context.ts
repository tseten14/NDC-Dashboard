...
import {
  buildCatalogFromFacts,
  buildDashboardFactLedger,
  factCitationGuide,
} from "@/lib/dashboard-ai-facts";
...
export interface DashboardAnalyzeContext {
  ...
  fact_ledger: ReturnType<typeof buildDashboardFactLedger>;
  quotable_facts: ReturnType<typeof factCitationGuide>;
}
...
export function buildDashboardAnalyzeContext(
  ...
  const factLedger = buildDashboardFactLedger(emissions, selectedTarget);
  const sourceCatalog = buildCatalogFromFacts(factLedger);
  ...
  return {
    ...
    quotable_facts: factCitationGuide(factLedger),
  };
}
